...
function sketch2(p5: P5CanvasInstance) {
  class Entit {
    zeroVec: Vector = p5.createVector(0, 0);
    slowVec: Vector = p5.createVector(-0.01, -0.01);
    ...
    move() {
      p5.keyIsDown(39) ? (this.acc.x += this.speed) : null;
      p5.keyIsDown(37) ? (this.acc.x -= this.speed) : null;
      p5.keyIsDown(38) ? (this.acc.y -= this.speed) : null;
      p5.keyIsDown(40) ? (this.acc.y += this.speed) : null;
      this.acc.limit(3);
...
export function Sketch2() {
  return <ReactP5Wrapper sketch={sketch2} />;
}
